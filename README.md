...
本项目是一个本地优先的 `ccusage` Token 用量仪表盘。它可以从本机或你已经配置好 SSH 访问的远端机器采集 coding agent 用量，统一写入本地 SQLite，并提供两个基于 Vue 的浏览器 UI（通过 SSE 实时推送更新，适合长时间常驻的展示屏）：

- 标准仪表盘：`/`
- 时钟仪表盘：`/clock`

服务端渲染的旧版仍保留在 `/legacy` 和 `/legacy/clock`。
...
- **主题**：札记 / 墨 / 雾 三档质感，控制栏可手动切换；不手动选时按时段自动切换（12:00–20:00 札记，21:00–06:00 墨，其余时间雾）。手动选择在当前标签页内保持，直到换一个或会话结束。
- **防烧屏**：常驻展示时整个画面会做缓慢的轻微像素偏移，避免静态内容烧屏。
- **周期切换**：今天 / 本周 / 本月 / 全年，模型与设备排名都按所选周期统计。
- 隐藏操作：双击标题 `Token 用量统计` 可切换浏览器全屏。

## 自动刷新用量

前端是基于 Vue 的单页应用：数据通过 `/api/view/*` 拉取，并订阅 `/api/events`（SSE）。每当后台采集写入 SQLite，服务端会推送一个更新信号，页面在约 3 秒内自动拉取最新数据并就地更新——**不整页重载、无闪烁**，适合长时间常驻的展示屏。连接断开后前端会自动重连（含心跳看门狗，以及设备唤醒 / 恢复网络时主动重连）。

要让数据本身也自动更新，`ui:serve` 内置了一个定时器，会按固定间隔在后台运行一次 `fleet:sync`，无需配置系统级的 cron / launchd / systemd。
